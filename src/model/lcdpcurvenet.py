    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_lambda_map = {
            ...
            SSIM_LOSS: lambda: kn.losses.ssim_loss(
                output_batch, gt_batch, window_size=5
            ),
        ...
        # If use preset to approximate output, add losses
        if self.opt[USE_PRESET]:
            preset_output_batch = self.net.res[PRESET_OUTPUT]
            loss_lambda_map.update(
                {
                    PRESET_L1_LOSS: lambda: self.pixel_loss(
                        preset_output_batch, gt_batch
                    ),
                    PRESET_COS_LOSS: lambda: 1
                    - F.sigmoid(self.cos(preset_output_batch, gt_batch).mean()),
                }
            )
...
class DeepWBNet(nn.Module):
    ...
    def __init__(self, opt=None):
        ...
        if self.opt[USE_PRESET]:
            self.preset_net = self.build_preset_net()
    ...
    def forward(self, x):
        ...
        if self.opt[HOW_TO_FUSE] == "cnn-weights":
            ...
            fused_x = torch.cat([x, brighten_x1, darken_x1], dim=1)
            weight_map = F.softmax(
                self.out_net(fused_x), dim=1
            )  # <- 3 channels, [ N, 3, H, W ]
            w1 = weight_map[:, 0, ...].unsqueeze(1)
            w2 = weight_map[:, 1, ...].unsqueeze(1)
            w3 = weight_map[:, 2, ...].unsqueeze(1)
        ...
        if self.opt[USE_PRESET]:
            fused_tensor = torch.cat([x, out], dim=1)
            preset = self.preset_net(fused_tensor)
            preset_output = preset_transform(x, preset)
            self.res.update(
                {
                    PRESET: preset,
            ...
            if self.opt[VISUALIZE_DEBUGS]:
                preset_brighten = preset_transform(x, preset[:, 0:1])
                preset_darken = preset_transform(x, preset[:, 1:2])

                self.res.update(
                    {
                        PRESET_BRIGHTEN: preset_brighten,
                        PRESET_DARKEN: preset_darken,
                    }
                )
